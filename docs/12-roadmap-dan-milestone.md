...
### M0 — Fondasi ✅ (selesai 2026-09-25)

- [x] Laravel 13 + Inertia v3 + React 19 + TS strict. Pest, PHPStan, lint, typecheck, dan e2e dijalankan di CI (`.github/workflows/tests.yml`).
- [x] PHPStan level max lulus **tanpa baseline**.
- [x] Extension `ltree`, `pg_trgm`, `pgcrypto`, `citext` aktif lewat migration.
- [x] `PlatformBootstrapSeeder` membuat root Pemda, app `core`, entity Core `organization`, role bawaan, dan admin pertama dalam satu transaksi (idempotent).
- [x] Login + 2FA (TOTP) wajib untuk area admin, rate limit login 5/menit per email+IP.
- [x] CRUD organisasi dengan `path` ltree. Pindah induk memperbarui path turunan dan `objects.owner_path`. Nonaktifkan (bukan hapus).
- [x] `AuditLogger` dan `EventRecorder` (tabel outbox, belum ada relay) tersedia sebagai kontrak. Audit append-only ditegakkan trigger DB.
- [x] Arch test: batas modul (hanya lewat `Contracts`), strict types, tanpa fungsi debug/berbahaya.
- [x] Axe test (WCAG 2.2 A/AA) untuk login & organisasi = 0 pelanggaran, di desktop, mobile, dan mode gelap.

Catatan implementasi M0:

- PHP 8.4 dan 8.5 sama-sama didukung (CI menguji keduanya), supaya Laragon di Windows bisa memakai 8.4.
