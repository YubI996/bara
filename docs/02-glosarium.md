...
Prinsip *Semantics Matter*: kesamaan nama belum berarti kesamaan konsep. Kalau ragu, tambahkan istilah baru ke sini dulu sebelum membuat entity.
...
| **Entity** | Tipe objek bisnis (mis. `Program`, `Realization`). Punya *storage type*. | `entities` |
...
| **System Entity** | Entity milik platform (Pentahelix, dsb.). Skema terkunci, hanya boleh ditambah *extension field*. | `entities.is_system` |
